...
- Shipping: Free on orders ETB 2,750+; otherwise flat ETB 330. Customer enters address at Checkout.
...
function pickProvider() {
  ...
  if (pref === "groq" && !hasGroq && hasGemini) return "gemini";
  if (pref === "groq" && !hasGroq && hasOpenAI) return "openai";
  if (pref === "openai" && !hasOpenAI && hasGroq) return "groq";
  if (pref === "openai" && !hasOpenAI && hasGemini) return "gemini";
  if (pref === "gemini" && !hasGemini && hasGroq) return "groq";
  if (pref === "gemini" && !hasGemini && hasOpenAI) return "openai";
  return null;
}
...
async function buildCatalogText() {
  ...
  return products
    .map((p) => {
      ...
      return [
        `- ${p.title}`,
        `  id: ${id} | ETB ${Number(p.price).toFixed(2)}` +
          (p.originalPrice > p.price ? ` (was ETB ${Number(p.originalPrice).toFixed(2)})` : ""),
...
async function chatWithGroq(system, userPayload, history) {
...
async function chat(req, res) {
  ...
  if (!provider) {
    return res.status(503).json({
      ...
      fallback: true,
      hint: "Set GROQ_API_KEY, GEMINI_API_KEY, and/or OPENAI_API_KEY in backend .env (see .env.example).",
    });
  }
  ...
  try {
    ...
    if (provider === "groq") {
      try {
        reply = await chatWithGroq(systemInstruction, userPayload, history);
      } catch (groqErr) {
